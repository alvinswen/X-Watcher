...
@pytest.mark.asyncio
async def test_cross_mode_search_equivalence(monkeypatch, tmp_path):
    """同数据 file vs sqlalchemy(SQLite)产同 (items 除 db_created_at, total)。ASCII keyword;
    distinct created_at 避 tie-break;created_at 按 instant 比 + 钉 file aware;db_created_at 双面断言。"""
    now = datetime.now(timezone.utc)
    base = now - timedelta(days=1)
    since = base - timedelta(hours=1)
    until = base + timedelta(hours=1)
    # text/ref 混合;t2 有 summary
    tw_specs = [("t1", "alice", base + timedelta(minutes=1), "alpha gpt", None),
                ("t2", "alice", base + timedelta(minutes=2), "beta", "ref gpt here"),
                ("t3", "bob", base + timedelta(minutes=3), "gamma gpt", None)]
    sum_tids = ["t2"]
    monkeypatch.setenv("XWATCHER_DATA_ROOT", str(tmp_path))
    await _seed_file(tmp_path, [_tweet(t, a, c, text=x, ref_text=r) for (t, a, c, x, r) in tw_specs],
    ...
    for (t, a, c, x, r) in tw_specs:
        session.add(TweetOrm(tweet_id=t, text=x, created_at=c, db_created_at=now,
                             author_username=a, author_display_name=f"{a} disp",
                             referenced_tweet_text=r, media=None))
    for t in sum_tids:
    ...
    cases = [dict(q="gpt", since=since, until=until),
             dict(q="gpt", since=since, until=until, include_summary=False),
             dict(q="gpt", author="alice", since=since, until=until),
             dict(q="gpt", page=1, page_size=2, since=since, until=until)]
    for kw in cases:
        monkeypatch.setenv("XWATCHER_DATA_LAYER", "file")
        f = await get_search_repo().search_tweets(**kw)
        monkeypatch.setenv("XWATCHER_DATA_LAYER", "sqlalchemy")
        s = await get_search_repo(session).search_tweets(**kw)
        assert f.total == s.total, f"total 不等 {kw}: file={f.total} sql={s.total}"
