...
def _utc_date_of(record: dict) -> date:
    return paths.as_utc(datetime.fromisoformat(record["created_at"])).date()


async def by_day_upsert(data_root: Path, records: list[dict]) -> None:
...
def read_by_day_dates(data_root: Path, utc_dates: list[date]) -> list[dict]:
...
def read_by_day_range(data_root: Path, since: datetime, until: datetime) -> list[dict]:
...
def reconcile_by_day(data_root: Path) -> tuple[bool, dict]:
