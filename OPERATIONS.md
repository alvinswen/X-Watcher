...
## by-day 派生视图体检、重建与 CHG-058 观察期

### 体检与治疗命令

`x-watcher views check` 是只读体检：它对账推文正本与 by-day 派生副本，先输出正本/副本计数及「正本有副本无、
副本有正本无、内容不一致、归档日错位」四类汇总。一致时退出码为 `0`；不一致时退出码为 `1`。默认每类只列
前 20 条，确需完整明细时再运行 `x-watcher views check --full`。

`x-watcher views rebuild` 是治疗动作：它会重写 by-day 派生副本，执行前先打印数据根、重写目录、现场记录路径和
当前分片数，并要求人工确认。无人值守窗口可显式使用 `x-watcher views rebuild --yes`。推荐闭环如下：

```bash
x-watcher views check
x-watcher views rebuild
x-watcher views check
```

成功判据是末次体检输出 `结论: 一致` 且退出码为 `0`。副本整个损坏时明细可能达到 6 万行，应先看四类计数汇总，
确认需要定位具体记录后再加 `--full`，避免终端被大量明细淹没。

### 恢复路径与强制重建兜底

1. 启动重建失败：服务仍会继续启动，并留下 error 日志及调用栈。先排除磁盘、权限等根因，再运行
   `x-watcher views rebuild`，最后用 `x-watcher views check` 复核。
2. 一条推文查到两条摘要：这是摘要一致性错误，不会静默选一条。先运行上一节「同一推文多条摘要自查 runbook」
   定位并按其步骤修复、重启 MCP 与 REST；随后运行 `x-watcher views check`。若同时发现 by-day 不一致，再运行
   `x-watcher views rebuild` 治疗派生副本。`views rebuild` 本身不会删除重复摘要。
3. 日志出现「写入 by-day 现场记录失败」：本轮副本已完成，失败只会导致下次启动多重建一次；这是可自愈警告。
   若连续出现，修复目录权限后手工运行 `x-watcher views rebuild`。

双指纹存在已登记的「内容改变但 mtime 与字节数都不变」盲区。需要无条件绕过跳过判据时，可只删除精确文件
`<data_root>/_views/by-day.state.json` 后重启服务；不要删除 `_views/by-day/` 整个目录。下次启动会因现场记录缺失
强制重建。也可直接运行 `x-watcher views rebuild`，它不经过跳过判据。

### 摘要写入与实例导入的预期代价

- 每保存一条摘要会重新读取定位信息，当前库存量级约多花 `0.31` 秒；稳态一轮 45 条约多花 14 秒，约占单轮
  14 分钟抓取的 `1.7%`。这是正确性加固的预期代价，不是故障；代价会随摘要库存线性增长，库存翻倍时每条约
  `0.62` 秒。为控制 Agent 调用时长，单次 `save_summaries` 建议不超过 50 条（批量提交走文件通道 `summaries_file` + `file_sha256`，单个交接文件同样建议 ≤50 条；参数通道 `summaries` 仅限单条修补）。
- 「摘要撞车拦截」是 warning 日志，但本次操作确实成功：本次内容已合并并覆盖已有那条的可变字段。撞车信息只在
  服务端日志，不进入 Agent 返回体，也不记录摘要正文、译文或凭据。
- 实例导入使用批量定位表，默认段长为 500。收尾 INFO 日志会记录「段长、刷新次数、条数」，用于事后回查撞车
  时点；导入结束后定位表会被丢弃，不会回填进全局缓存。

### 观察期判读

启动日志写「跳过原因=双指纹一致」表示本次可安全跳过；写「触发原因=推文正本变了/按天索引副本被改动/
现场记录缺失/现场记录读不出/现场记录版本不认识」表示已进入重建。完成日志会给出产出天数与清理幽灵天数。
重建采用逐天原子替换：任一存在的天文件始终完整，但重建进行中不同日期可能短暂处于新旧混合态；双冷启结束后
必须以 `views check` 的整体一致为准。抓取与冷启恰好并发导致新推文被旧快照覆盖时，只能等待下一次正本变化触发
启动重建，或立即运行 `x-watcher views rebuild`；增量 `by_day_upsert` 只能写自己的批次，不能补回别的批次。
