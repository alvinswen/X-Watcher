...
| 2026-06-24 | file 模式下 PG/Docker 已可下线，但 CLAUDE.md Step 0 仍要求"先起 Postgres" | 抓取前置检查清单 Step 0「确认 OrbStack + Postgres 已运行」+ drift-log 2026-06-07 条：会话启动前必须先起 Postgres，否则 x-watcher MCP 握手静默失败、全程无 MCP 工具 | file 模式（`XWATCHER_DATA_LAYER=file`）下 `src/mcp/lifespan.py:73` 守卫让 MCP 启动跳过 init_database/async engine 预热、**根本不连 PG**；`main.py:75` / `async_session.py:59` 同有 pg 下线守卫；PG 容器实测 `\dt` 已无任何表（全 DROP），数据全在 `data_migrated/`（11 域齐全）。即 Step 0「先起 Postgres」在 file 模式已不成立 | 已清运行态：删空跑的 `x-watcher-postgres` 容器 + `x-watcher_postgres-data` volume；`.env` 移除 PG `DATABASE_URL`（回落 sqlite 默认）；`docker-compose.yml` 删 prod profile（postgres + app-prod）。CLAUDE.md Step 0 已改写（owner 2026-06-24 授权） | 【已闭环】Step 0（LOCKED 段 64-100）已改写为"file 模式无需 Postgres/Docker，直接启会话即可"，旧 precheck 降级为"仅切回 `sqlalchemy` 模式才适用"；同段 line 68 顺带去掉过时的 `claude-opus-4-7` 硬编码（改指 `config.py:103 claude_code_model_name`）。注：CLAUDE.md 被 .gitignore 忽略，本地生效不入库 |
| 2026-06-26 | `1cf33d7` 下线调度器后 `pyproject.toml` 仍声明 apscheduler 死依赖 | 用户报告：让 Claude Code 起后台服务时会话里出现 `apscheduler`，疑似"调度器/数据库又被启动" | `1cf33d7` 已删调度器全部源码(130 文件)且全库零 `import apscheduler`；但 `pyproject.toml:43 apscheduler>=3.10.0` + mypy override `apscheduler.*` 漏删，`.venv` 仍装 3.11.2，且无 `uv.lock` → 每次 `uv run`/`uv sync` 从 pyproject 重新解析依赖，把 apscheduler 包名打印出来被误读为调度器回潮；运行期从不加载，日志证实 file 模式不连库、无调度活动 | 已改代码 | 删 pyproject apscheduler 声明 + mypy override；`pip uninstall apscheduler`(3.11.2)；清 4 处过时 APScheduler 注释(scheduled_job/scraping_service/summarization_queue)；删孤立死代码(metrics `scheduler_job_*` 4 指标 + schemas 3 个 `Schedule*` 模型)；import + schema 测试通过(10 passed)。另发现独立隐患：`.env` 缺 `JWT_SECRET_KEY`，只读 .env 的新进程(MCP)会 `sys.exit(1)`，待 owner 拍板补 key |
